...
import type { DailyCleaningTask, Profile, Store } from "@/lib/types";
...
async function fetchImageAsBase64(url: string, maxWidth = 600, quality = 0.5): Promise<string | null> {
  try {
...
async function generateDailyCleaningAreaSummaryPDFReport(
  stores: any[],
  tasks: DailyCleaningTask[],
  managerName: string,
  reportDate: string
  ...
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text("Daily Cleaning Area Summary Report", margin, 32);

  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(71, 85, 105);
  doc.text(`Area Manager: ${managerName}`, margin, 38);
  doc.text(`Tanggal Daily Cleaning: ${reportDate} • Lingkup: Seluruh Store Area`, margin, 43);

  const sumStorePct = stores.reduce((acc, st) => {
    const storeTasks = tasks.filter((t) => t.store_id === st.id);
    const totalT = storeTasks.length;
    const completedT = storeTasks.filter((t) => t.status === "completed").length;
  ...
  doc.setFontSize(10);
  doc.setTextColor(30, 41, 59);
  doc.text("Summary Report Daily Cleaning All Stores", margin, y);
  y += 4;
  ...
  const fileName = `DailyCleaning_Area_Summary_${reportDate.replace(/\//g, "-")}.pdf`;
  doc.save(fileName);
...
export default function DailyCleaningPage() {
  ...
  const loadData = useCallback(async () => {
    if (!effectiveStoreId && !isSuperAdmin && !isAreaManager) return;
    ...
    const [data, members, stores] = await Promise.all([
    ...
    setLoading(false);
  }, [effectiveStoreId, isSuperAdmin, isAreaManager, activeStoreId, profile?.store_id, selectedDate]);
  ...
  if (isAreaManager) {
    ...
    const completedAreaTasks = tasks.filter((t) => t.status === "completed").length;
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100 flex items-center gap-2">
              ...
              Ringkasan Daily Cleaning per-Store
            </h2>
            <p className="text-sm text-slate-500">
              Pantau jumlah tugas, progres update, dan lihat laporan dari semua toko di area Anda.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button
              onClick={async () => {
                setGeneratingPdf(true);
                try {
                  await generateDailyCleaningAreaSummaryPDFReport(
                    allStores,
                    tasks,
                  ...
                  console.error("Error generating Area PDF:", err);
                  alert("Gagal memuat laporan PDF Area: " + (err instanceof Error ? err.message : String(err)));
                } finally {
                  setGeneratingPdf(false);
                }
              }}
              disabled={generatingPdf || allStores.length === 0}
              className="bg-blue-600 hover:bg-blue-700 text-white h-9 text-xs sm:text-sm gap-2 font-medium"
            >
              {generatingPdf ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm">
            <div className="text-xs text-slate-500 font-medium">Total Tugas Area</div>
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">{totalAreaTasks} Tugas</div>
          </div>
          <div className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm">
                      ...
                      <tr key={st.id} className="hover:bg-slate-50/60 dark:hover:bg-zinc-800/40 transition-colors">
                        <td className="px-6 py-4 font-bold text-slate-800 dark:text-white">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-xs shrink-0">
                        ...
                        <td className="px-6 py-4 w-56">
                          <div className="flex items-center gap-2">
                            <div className="h-2 flex-1 bg-slate-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                              <div
                                className={`h-full ${pct === 100 ? "bg-emerald-500" : pct > 0 ? "bg-blue-500" : "bg-slate-300 dark:bg-zinc-700"}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          ...
                          {hasReport ? (
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                if (pdfWindow) {
                                  pdfWindow.document.write("<html><head><title>Membuat Laporan Daily Cleaning...</title></head><body style='font-family:sans-serif;padding:2rem;text-align:center;'><h3>Sedang membuat laporan Daily Cleaning PDF...</h3><p>Mohon tunggu sebentar.</p></body></html>");
                                }
                                setGeneratingPdf(true);
                                try {
                                  const { blob, fileName } = await generateDailyCleaningPDFReport(
                                    storeTasks,
                                    st.name || "Toko",
                                    st.code || null,
                                    profile?.full_name || "Area Manager",
