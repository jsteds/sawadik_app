...
async function generatePDFReport(
  ...
  doc.text("General Cleaning Report", margin, 32);

  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(71, 85, 105);
  doc.text(`${storeName}${storeCode ? " (" + storeCode + ")" : ""}`, margin, 38);
  doc.text(`Tanggal GC: ${reportDate}`, margin, 43);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    // Right: Blue shape (thinner, more gentle slope)
    ...
    // Text
    doc.setFontSize(7);
    doc.setTextColor(255, 255, 255); // White on the blue curve
    doc.text(`Halaman ${p} dari ${totalPages}`, pageWidth - margin, pageHeight - 4, { align: "right" });
    doc.setTextColor(148, 163, 184); // Gray on the white background
