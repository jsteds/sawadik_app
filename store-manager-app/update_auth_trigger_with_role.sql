-- Script untuk memperbarui trigger pendaftaran agar membaca 'role' dari form pendaftaran.
...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'nik' IS NOT NULL AND NEW.raw_user_meta_data->>'nik' != '' THEN
    ...
    SET auth_user_id = NEW.id,
        ...
        role = COALESCE(NEW.raw_user_meta_data->>'role', role) -- Timpa role jika form mendaftarkan role spesifik
  ...
  IF linked_profile_id IS NULL THEN
    INSERT INTO public.profiles (auth_user_id, email, full_name, nik, role)
    VALUES (
      NEW.id, 
      NEW.email, 
      COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email),
      NEW.raw_user_meta_data->>'nik',
      COALESCE(NEW.raw_user_meta_data->>'role', 'staff') -- Ambil role dari pendaftaran, jika kosong jadikan staff
    );
  END IF;
